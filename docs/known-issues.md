...
### DP-005 产品第四阶段目标环境与生产成熟度门禁未关闭

- **发现日期：** 2026-09-08
- **状态：** 转第四阶段，阻塞（待完成验收，不是未开发功能）
- **影响范围：** 第四阶段目标部署兼容性、数字人生产成熟度和长期运维验收；不再阻塞第二、三阶段功能完成
- **阶段调整：** 2026-09-09 用户明确将四项目标环境门禁移交第四阶段，并将第二阶段置为完成；2026-09-12 又将数字人真实供应商、播放矩阵、性能和生产运维门禁纳入第四阶段；此次调整不改变测试执行事实。
- **现象：** 真实密码管理器、将要上线的完整代理链、物理 4K 设备、连续 8 小时播放，以及数字人真实供应商、播放矩阵、性能基线和生产运维没有目标环境证据。生产等价 Nginx、真实 Retina、4K 仿真、fake TTS 和 10 分钟浸泡只能作为预检。
- **已有证据：** `test-evidence/blackbox-20260909/proxy-equivalent/`、`display-target/`、`soak-preflight-10m-final/`。
- **2026-09-21 条件确认：** 用户明确回复“尚未准备，先完成开发和本地验证”。本轮开发与本地验证已完成；最终后端 848 项、三浏览器完整 E2E 78 项及 50 客户端容器恢复演练通过。本地通过不改变八项目标门禁状态，结果见 `docs/PHASE_FOUR_CLOSEOUT.md`。
- **后续动作：** 按 `docs/PHASE_FOUR_TARGET_ENVIRONMENT.md` 的 P4-ENV-001～004 和 P4-DH-001～004 执行，逐项补齐目标证据后关闭本记录；预检不能计为第四阶段目标环境或数字人生产成熟度验收通过。

## 已解决

### DP-008 同一文件并发查询重复解析导致 500

- **发现日期：** 2026-09-21
- **状态：** 已解决（固定资源的完整容器复跑通过）
- **现象：** 50 个显示 Session 同时查询同一 CSV，每次查询重复启动解析器，超过两个处理槽后出现未映射错误。
- **修复：** 有界缓存合并同文件解析，源文件和规范化文件变化时失效；繁忙返回 429、解析无效返回 422；重复取消等待子进程回收，不提前释放处理槽。
- **证据：** `test-evidence/phase-four/local-20260921/container-recovery-retry/result.json` 保留失败；最终 `container-final/result.json` 250/250 查询成功、p95 590 ms，2 CPU/4 GiB/256 PID 与 3,000 ms 门槛不变。27 项处理器定向回归及最终后端全量 848 项通过。
