# DataPulse `E3`：文件数据集与 AI 分析设计

**状态：** 已确认范围

**日期：** 2026-08-01

**产品阶段归属：** 产品第二阶段（数据理解与融合创作）；本文件的“第三阶段”仅指历史工程命名，现已由产品数字人播报阶段替代。

**前置阶段：** DataPulse 产品第一阶段的数据源工作台、大屏编辑/播放/安全嵌入
...
2. 让用户可以用自然语言进行数据分析、生成图表建议或生成完整大屏草稿，AI 输出结构化 `AnalysisPlan` / `ChartSpec` / `DashboardDocument`，再复用现有查询安全层和受注册表约束的组件展示；原始 E3 设计时为九类，当前为 21 类。

本阶段不扩展组件种类、不做设备管理、不做插件市场；允许 AI 使用现有组件生成一张可编辑的草稿大屏，但不允许自动发布。

## 2. 已有能力与本工程里程碑边界

原始 E3 设计以当时九类内置组件为边界；产品第二阶段后续已将注册表扩展为当前 21 类。本工程里程碑只补充直接影响文件数据/AI 体验的组件属性、空状态和错误状态，不单独扩大组件协议；AI 输出始终按当前注册表白名单校验。
...
2. 文件数据集可以预览、编辑名称、删除，并绑定到当前 21 类组件中具备对应数据能力的组件。
...
8. 用户可以输入一句话生成只使用当前 21 类注册组件的完整大屏草稿，确认后才创建草稿记录；生成结果不能直接发布。
