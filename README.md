...
- **第二阶段：数据理解与融合创作。** 文件数据集、DuckDB、AI 分析、整屏草稿生成、模板、演示/静态数据和编辑器内 AI 局部修改。
- **第三阶段：完整数字人播报。** 以数字人组件为载体，提供数据驱动的话术、字幕、播报状态、定时/事件触发、语音播放、静音/暂停和播放/嵌入兼容；必须支持无 AI、无音频时的可用降级。
- **第四阶段：产品成熟度与生态。** 多用户协作、复杂权限、模板/组件市场、插件 SDK、备份恢复、规模化质量和长期运维能力。

当前映射为：`E1`/`E2` 已完成第一阶段核心能力；`E3`/`E4` 已交付第二阶段首批能力，
截至 2026-09-08，第二阶段定义的 8 组功能均已有实现，确认未开发功能点为 0；但 Firefox
和 WebKit 编辑器视觉门禁仍失败，真实密码管理器、生产反向代理/CSP、4K/高 DPI 和长期
播放仍缺少目标环境，因此第二阶段尚不能标记完成。详见
[`docs/PHASE_TWO_CLOSEOUT.md`](docs/PHASE_TWO_CLOSEOUT.md)。第三阶段尚未开始，第四阶段也尚未开始。数字人播报不阻塞当前
核心发布，需在进入第三阶段时单独建立协议、运行时、媒体资源、触发策略和质量门禁。

产品路线的阶段验收不能仅以工程里程碑完成作为依据。第二阶段仍需关闭跨浏览器视觉、
目标部署安全边界、高 DPI 和长期播放门禁；数字人播报的可用性与资源占用在第三阶段验收。
